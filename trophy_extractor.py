...
_AWARD_CEREMONY_KEYWORDS = [
    ...
    "케이월드드림어워즈", "K-월드 드림 어워즈", "KWDA", "K WORLD DREAM AWARDS", "K-WORLD DREAM AWARDS",
    "지니뮤직어워드",
    "SBS가요대전", "SBS 가요대전",
    "MBC가요대제전", "MBC 가요대제전",
    "KBS가요대축제", "KBS 가요대축제",
...
def _match_award_type(title):
    # "베스트 팬덤상"처럼 "베스트"와 "팬덤상"이 둘 다 매칭될 수 있는 경우,
    # 짧고 범용적인 "베스트"보다 더 구체적인 "팬덤상"을 우선 선택.
    # 길이가 같으면(둘 다 3글자 등) 목록에서 더 뒤에 있는, 더 구체적인
    # 항목을 우선하도록 순서도 같이 고려함
    matches = [t for t in _AWARD_TYPE_KEYWORDS if t in title]
    if not matches:
        return None
    return max(matches, key=lambda t: (len(t), _AWARD_TYPE_KEYWORDS.index(t)))
