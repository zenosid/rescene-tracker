...
def build_awards():
    """시상식 수상 기록. 수동 등록(과거 놓친 것)과 자동 감지를 합쳐서 보여줌."""
    from datetime import date as _date_cls
    ...
    manual_items = [
        {
            ...
            "title": a.get("note", f"{a['ceremony']} {a.get('award_name', '')}"),
            "source_link": "",
            "is_manual": True,
        }
    ...
    # 수동 등록과 (시상식, 상 이름)이 같고 날짜도 3일 이내로 가까우면 자동
    # 감지 쪽은 중복이니 빼고 수동 등록(더 신뢰할 수 있는 쪽)만 남김
    def _is_near_manual(auto_item):
        for m in manual_items:
            if m["ceremony"] != auto_item["ceremony"] or m["award_name"] != auto_item["award_name"]:
                continue
            try:
                d1 = _date_cls.fromisoformat(m["date"])
                d2 = _date_cls.fromisoformat(auto_item["date"])
