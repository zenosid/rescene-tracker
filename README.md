...
- **🎂 기념일 D-day**: 데뷔일(공식 확인, 2024.03.26) + 멤버 생일(직접 등록)까지
  남은 날짜를 스케줄 탭 상단에 카운트다운으로 표시.
- **🏆 트로피 탭**: 뉴스 기사에서 "음악방송명 + 1위" 패턴을 자동 감지해서
  수상 기록을 모아봅니다.
- **📦 소장품 탭**: 포토카드·앨범 소장 현황을 보유/위시리스트로 관리 (브라우저
  localStorage 저장, 즐겨찾기와 같은 방식).
- **🌟 하이라이트 탭**: X·인스타그램 게시물을 자동 검색은 못 하지만, 직접 고른
  특정 게시물을 각 플랫폼의 공식 임베드 위젯으로 그대로 보여줍니다 (스크래핑
  아님, 좋아요 수·프로필까지 실제 그대로 표시).
...
- **🗂️ 아카이브**: 날짜별로 묶은 유튜브(공식/콜라보)·뉴스(구글+네이버)·카페글/블로그.
  소스·카테고리·멤버별
  필터, 각 카드에 즐겨찾기·공유 버튼.
...
- **🏆 트로피**: 음악방송 1위 수상 기록 (뉴스 기사 자동 감지).
- **📦 소장품**: 포토카드·앨범 소장 현황 (개인 브라우저 저장).
- **🌟 하이라이트**: 직접 고른 X·인스타그램 게시물 공식 임베드.
...
## 6. 네이버 뉴스·카페글·블로그 검색 기능 켜기 — 선택사항

구글 뉴스 RSS 외에 네이버 뉴스·카페글·블로그도 같이 수집하고 싶으면 네이버
오픈 API가 필요합니다 (무료, **하나의 키로 셋 다 됩니다**).
...
카페글 검색은 리시안셔스뿐 아니라 **검색에 공개된 모든 네이버 카페**를 대상으로
합니다 — 브랜드 콜라보 후기처럼 뉴스로는 잘 안 잡히는 소식을 잡기 위한 용도입니다.
블로그 검색도 같은 이유로 추가했습니다. 역시 키가 없으면 이 기능만 건너뛰고
구글 뉴스는 그대로 작동합니다.
...
├── trophy_extractor.py                 # 뉴스 → 음악방송 1위 기록 추출
