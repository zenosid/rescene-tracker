...
# ── 저장소 ──────────────────────────────────────────────────────
# ── X(트위터) 검색 (공식 API, 유료) ──────────────────────────
# 2026.02부터 X API가 완전 종량제(조회 1건당 $0.005)로 바뀌어서 유료입니다.
# developer.x.com에서 앱 만들고 크레딧 충전 후 Bearer Token 발급.
# 보안을 위해 이 파일에 직접 적지 말고 환경변수로 설정해주세요:
#   로컬:  $env:X_BEARER_TOKEN="여기에_토큰"  (PowerShell)
#   배포:  저장소 Settings → Secrets and variables → Actions → New repository secret
#          이름: X_BEARER_TOKEN
# 토큰이 없으면 이 기능은 조용히 건너뜁니다 (다른 기능엔 영향 없음).
X_SEARCH_QUERIES = ["리센느", "RESCENE"]
X_SEARCH_MAX_RESULTS = 20  # 검색어당 최대 몇 건 (비용과 직결되니 너무 크게 하지 마세요)

# ── 모더레이션 감지 (악플/허위사실 후보, 운영자 전용) ────────────────
# 자동으로 신고하지 않습니다 — 이미 수집된 뉴스/커뮤니티/X 글 중에서 아래
# 키워드가 제목에 있으면 moderation_flags 테이블에 기록하고, GitHub Actions
# 실행 로그(운영자만 볼 수 있음)에 경고로 표시합니다. 실제 신고 여부는 직접
# 내용을 확인하시고 각 플랫폼의 정식 신고 기능으로 판단해서 진행해주세요.
# 이 목록은 예시이니 필요에 따라 자유롭게 수정하세요.
MODERATION_KEYWORDS = [
    # 허위사실/조작 의혹
    "사재기", "조작설", "조작 의혹", "뒷광고", "표절", "짜집기", "도용 의혹",
    # 논란/자질 시비
    "논란", "퇴출", "학폭", "갑질", "인성논란", "빌런",
    # 외모/실력 비하 (팬덤 내에서도 자주 오남용되는 표현이라 오탐 가능성 있음)
    "성괴", "발연기", "무개념",
    # 일반 비하성 표현
    "역겹", "혐오스럽", "재수없", "꼴보기싫",
    # 루머/명예훼손
...
# 참고: 위 목록은 "이런 표현이 있으면 사람이 한번 검토해볼 만하다"는 신호일
# 뿐입니다. 특히 "논란"이나 "인성논란" 같은 단어는 실제로 문제 있는 글이
# 아니라 그냥 그 이슈를 다루는 일반 기사에도 흔히 쓰이니, 걸렸다고 무조건
# 나쁜 글은 아닙니다 — 꼭 직접 열어서 확인하고 판단해주세요. 정말 심한
# 수위의 욕설/비속어는 이 파일에 직접 적어두지 않았으니, 필요하시면
# 아래 리스트에 원하시는 단어를 자유롭게 추가하시면 됩니다.

# ── 저장소 ──────────────────────────────────────────────────────
DB_PATH = "rescene_tracker.db"

# config.py 맨 아래에 그대로 붙여넣어주세요

# ── 시상식 (수동 등록, 최우선 신뢰) ──────────────────────────────
# 뉴스 자동 수집은 "최근 기사"만 가져오는 구조라, 저희가 실제로 수집을
# 시작하기 전에 받은 상은 자동으로는 절대 못 잡습니다. 놓친(또는 과거)
# 수상 기록은 여기에 직접 추가해주세요. date는 'YYYY-MM-DD' 형식.
AWARD_ITEMS = [
    {"date": "2024-11-02", "ceremony": "아시아 모델 어워즈", "award_name": "라이징스타상",
     "note": "ASIA MODEL AWARDS 라이징스타상 수상"},
    {"date": "2025-08-28", "ceremony": "뉴시스 한류엑스포", "award_name": "한류특별상",
     "note": "제7회 뉴시스 한류엑스포 한류특별상 수상"},
    {"date": "2026-01-06", "ceremony": "대한민국 퍼스트브랜드 대상", "award_name": "여자아이돌(라이징스타)",
     "note": "2026 대한민국 퍼스트브랜드 대상 여자아이돌(라이징스타) 부문 수상"},
    {"date": "2026-08-27", "ceremony": "KWDA", "award_name": "K 월드 드림 베스트 비주얼 콘텐츠상",
     "note": "2026 K WORLD DREAM AWARDS 베스트 비주얼 콘텐츠상 수상"},
    {"date": "2026-08-27", "ceremony": "KWDA", "award_name": "K 월드 드림 베스트 팬상",
     "note": "2026 K WORLD DREAM AWARDS 베스트 팬상 수상"},
    {"date": "2026-09-01", "ceremony": "올해의 브랜드 대상", "award_name": "여자아이돌(핫트렌드)",
     "note": "2026 올해의 브랜드 대상 여자아이돌(핫트렌드) 부문 수상"},
    {"date": "2026-09-06", "ceremony": "SKA", "award_name": "GLOBAL CONNECT",
     "note": "2026 SPOTV K-POP AWARDS GLOBAL CONNECT 수상"},
    {"date": "2026-09-06", "ceremony": "SKA", "award_name": "SPOTV AMBASSADOR",
     "note": "2026 SPOTV K-POP AWARDS 앰버서더상 수상"},
    {"date": "2026-09-19", "ceremony": "TMA", "award_name": "올해의 아티스트",
     "note": "2026 THE FACT MUSIC AWARDS 올해의 아티스트 수상"},
]
